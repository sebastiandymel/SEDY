...
namespace FakeIMC.Server
{
    internal class Imc2ServerStub : MarshalByRefObject, IIMCServerEx, IImcServerFacade
    {
        ...
        public void LoadCurves(CurvesContainer container)
        {
            ...
            this.sii = new SiiWrapper(container.SiiAided, container.SiiUnaided);
            ...
            if (this.isSpeechMode)
            {
                ...
                {
                    // LOW:
                    LoadCurve(container.Percentiles30, GetPer30Key(Constants.Low), container.SpeechLowOffset);
                    // MEDIUM
                    LoadCurve(container.Percentiles30, GetPer30Key(Constants.Medium));
                    // HIGH
                    LoadCurve(container.Percentiles30, GetPer30Key(Constants.High), container.SpeechHighOffset);
                }
                if (container.Percentiles30 != null)
                {
                    // LOW
                    LoadCurve(container.Percentiles99, GetPer99Key(Constants.Low), container.SpeechLowOffset);
                    // MEDIUM
                    LoadCurve(container.Percentiles99, GetPer99Key(Constants.Medium));
                    // HIGH
                    LoadCurve(container.Percentiles99, GetPer99Key(Constants.High), container.SpeechHighOffset);
                }
        ...
        private void OnTaskFailed(string taskName, Task t)
        {
            if (t.IsFaulted)
            {
                string exMsg = string.Empty;
                if (t.Exception is AggregateException agEx)
                {
                    exMsg = string.Join(", ", agEx.InnerExceptions.Select(ie => ie.Message);
                }
                else
                {
                    exMsg = t.Exception?.Message;
                }
                WarnOut($"Task {taskName} failed: {exMsg}");
            }
        }
        ...
        private void BeginMeasurement()
        {
            ...
            Task.Factory
                .StartNew(SendEndOfMeasurement)
                .ContinueWith(t => OnTaskFailed(nameof(SendEndOfMeasurement), t), TaskContinuationOptions.OnlyOnFaulted);            
        }
        ...
        private void SendEndOfMeasurement()
        {
            ...
            DataReady rem = new DataReady
            {
                ...
                StatusText = this.isSpeechMode ? this.sii.AsJson(this.measurementConditions.Side) : "Data"
            };
        ...
        private void Send(DataReady data, string dataType)
        {
            ...
            {
                var serializedData = "";
                serializedData = JsonConvert.SerializeObject(data) + Environment.NewLine;
        ...
        {
            var serializedData = JsonConvert.SerializeObject(pvData);
                ...
                case IMCCommandEnum.ic_StartMeasurement:
                    {
                        if (NotRunning(ref result))
                        {
                            ...
                            Task.Factory
                                .StartNew(() => BeginMeasurement())
                                .ContinueWith(t => OnTaskFailed(nameof(BeginMeasurement), t), TaskContinuationOptions.OnlyOnFaulted);
                        }
                        break;
                    }
